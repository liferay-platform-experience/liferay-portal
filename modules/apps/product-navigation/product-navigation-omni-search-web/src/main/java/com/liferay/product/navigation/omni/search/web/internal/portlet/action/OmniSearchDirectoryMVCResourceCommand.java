...
@Component(
	property = {
		...
		"mvc.command.name=/omni_search/omni_search_directory"
	},
	service = MVCResourceCommand.class
)
public class OmniSearchDirectoryMVCResourceCommand
	...
	protected void doServeResource(
		...
		for (OmniSearchEntryProvider omniSearchEntryProvider :
				_omniSearchEntryProviders) {

			try {
				List<OmniSearchEntry> omniSearchEntries =
					omniSearchEntryProvider.getOmniSearchEntries(
						httpServletRequest, themeDisplay);

				for (OmniSearchEntry omniSearchEntry : omniSearchEntries) {
					itemsJSONArray.put(
						JSONUtil.put(
							"category", omniSearchEntry.getCategory()
						).put(
							"label", omniSearchEntry.getLabel()
						).put(
							"type", omniSearchEntryProvider.getType()
						).put(
							"url", omniSearchEntry.getURL()
						));
				}
			}
			catch (Exception exception) {
				_log.error(
					"Unable to get entries from " + omniSearchEntryProvider,
					exception);
			}
		}
	...
	private static final Log _log = LogFactoryUtil.getLog(
		OmniSearchDirectoryMVCResourceCommand.class);
	...
	private volatile List<OmniSearchEntryProvider> _omniSearchEntryProviders;
