...
const CONTENT_URL = '/omni-search-content?x=1';
const DIRECTORY_URL = '/omni-search-directory';
...
const mockFetchResponses = () => {
	(fetch as jest.Mock).mockImplementation((url: string) => {
		const response = url.includes('/omni-search-content')
			? CONTENT_RESPONSE
			: DIRECTORY_RESPONSE;
...
const openModal = async () => {
	render(
		<OmniSearch contentURL={CONTENT_URL} directoryURL={DIRECTORY_URL} />
	);
...
const searchFor = async (keywords: string) => {
	...
	await waitFor(() =>
		expect(fetch).toHaveBeenCalledWith(
			expect.stringContaining('/omni-search-content'),
			expect.anything()
		)
	);
...
describe('OmniSearch', () => {
	...
	it('shows only the current user recent searches', async () => {
		localStorage.setItem(
			'liferay-omni-search-recent-42',
			JSON.stringify(['welcome'])
		);
		localStorage.setItem(
			'liferay-omni-search-recent-99',
	...
	it('removes a recent search with its remove button', async () => {
		...
		expect(localStorage.getItem('liferay-omni-search-recent-42')).toBe(
			'[]'
		);
	});
	...
	it('keeps repeated entries as distinct options without key collisions', async () => {
		...
		(fetch as jest.Mock).mockImplementation((url: string) =>
			Promise.resolve({
				json: () =>
					Promise.resolve(
						url.includes('/omni-search-content')
							? {items: [], totalCount: 0}
							: {
									items: [
										{
	...
	it('renders a single button when several CMS toolbar instances mount', async () => {
		...
		expect(
			await screen.findAllByLabelText('omni-search (Ctrl+K)')
		).toHaveLength(1);
