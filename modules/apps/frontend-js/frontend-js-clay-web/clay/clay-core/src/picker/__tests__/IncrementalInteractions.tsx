...
describe('Picker incremental interactions', () => {
	...
	describe('expanded menu', () => {
		...
		it('pressing the down arrow key navigates past a disabled first option', () => {
			...
			// The visual focus starts on the disabled option, which is
			// reachable by keyboard even though it cannot be selected.
		...
		it('pressing the up arrow key keeps the visual focus on a disabled first option', () => {
			const {getByRole} = render(
				<Picker items={['Apple', 'Banana', 'Blueberry']}>
					{(item) => (
						<Option disabled={item === 'Apple'} key={item}>
		...
		it('pressing the tab key while a disabled option has visual focus closes the menu and reopening it highlights the selected option', () => {
			const {getByRole} = render(
				<>
					<Picker
		...
		it('pressing alt + up arrow while a disabled option has visual focus closes the menu and reopening it highlights the selected option', () => {
			const {getByRole} = render(
				<Picker
					defaultSelectedKey="Banana"
					items={['Apple', 'Banana', 'Blueberry']}
			...
			userEvent.keyboard('{Alt>}{ArrowUp/}');
