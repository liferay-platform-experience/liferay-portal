...
public class LayoutLocalServiceTest {
	...
	@Test
	@TestInfo("LPD-88081")
	public void testUpdateStyleBookEntryERCWithoutScopeClearsExistingScope()
		throws Exception {
		...
		_layoutLocalService.updateStyleBookEntryERC(
			layout.getGroupId(), layout.isPrivateLayout(), layout.getLayoutId(),
			"any-erc", "leftover-scope");

		Layout updatedLayout = _layoutLocalService.updateStyleBookEntryERC(
			layout.getGroupId(), layout.isPrivateLayout(), layout.getLayoutId(),
			"new-erc");

		Assert.assertEquals("new-erc", updatedLayout.getStyleBookEntryERC());
		Assert.assertTrue(
