...
const FileEntryPicker = ({
	...
	const setDangerFeedback = (message: string) => {
		setFeedback({displayType: 'danger', message});
	};

	const setInfoFeedback = (message: string) => {
		setFeedback({displayType: 'info', message});
	};
	...
	const validateFrontendTokenDefinitionFile = async (file: File) => {
		post(objectToFormData({file}), '/validate-file')
			.then(async (response) => {
	...
	const handleFileInputChange = async ({
		...
		setFrontendTokenDefinition(await readInputFile(file));
